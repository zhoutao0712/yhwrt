		...
		if(!value || 
			...
			if(!strcmp(name, "wl_unit")
				...
				if(strcmp(nvram_safe_get(tmp), value))
				{
					...
					_dprintf("set %s=%s\n", tmp, value);
#ifdef RTCONFIG_REALTEK
					if(nvram_match("sw_mode", "2") && strstr(prefix,".1_"))
					{//set root ap info
						strcpy(tmp+3,tmp+5);
						if(strcmp(nvram_safe_get(tmp), value))
						{
...
#endif
	generate_token(ifttt_token, sizeof(ifttt_token));
	add_asus_token(asus_token);
...
int check_xss_blacklist(char* para, int check_www)
{
	...
	if(para == NULL || !strcmp(para, "") || (strlen(para) > sizeof(para_str))){
