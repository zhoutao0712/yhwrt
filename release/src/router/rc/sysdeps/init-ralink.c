	...
	if (is_routing_enabled())
	{
		...
		if(!nvram_match("switch_wantag", "none")&&!nvram_match("switch_wantag", ""))//2012.03 Yau modify
		{
			...
			case MODEL_RTAC53:
				...
				system("rtkswitch 40 1");			/* admin all frames on all ports */
				/* Internet/STB/VoIP:	untag: N/A;   port: P0, P9 */
				__setup_vlan(6, 0, 0x00000201);
				__setup_vlan(2, 0, 0x00000201);
				__setup_vlan(3, 0, 0x00000201);
#endif
				}
#endif
...
set_wan_tag(char *interface) {
...
#ifdef RTCONFIG_MULTICAST_IPTV
	{
		...
		if (switch_stb >= 7) {
			system("rtkswitch 40 1");			/* admin all frames on all ports */
			system("rtkswitch 38 1");
			if(wan_vid) { /* config wan port */
#if defined(RTAC51UP)
				__setup_vlan(wan_vid, 0, 0x00000201);	/* config WAN & WAN_MAC port */
				_set_vlan_mbr(wan_vid);
...
#if defined(RTAC51UP)
				__setup_vlan(iptv_vid, iptv_prio, 0x00000201);	/* config WAN & WAN_MAC port */
				_set_vlan_mbr(iptv_vid);
#else
...
#if defined(RTAC51UP)
				__setup_vlan(voip_vid, voip_prio, 0x00000201);	/* config WAN & WAN_MAC port */
				_set_vlan_mbr(voip_vid);
#else
...
#if defined(RTAC51UP)
				__setup_vlan(mang_vid, mang_prio, 0x00000201);	/* config WAN & WAN_MAC port */
				_set_vlan_mbr(mang_vid);
#else
