...
if(test_log) _dprintf("wanduck: %s: get_wanports_status(%d)=%d\n", __func__, wan_unit, link_wan[wan_unit]);
...
#if defined(RTAC51UP)
	// the first value of get_wanports_status() was incorrect.
	// So, do it once here.
	for(wan_unit = WAN_UNIT_FIRST; wan_unit < WAN_UNIT_MAX; ++wan_unit)
		if(get_dualwan_by_unit(wan_unit) == WANS_DUALWAN_IF_WAN){
			get_wanports_status(wan_unit);
			break;
		}
#endif
