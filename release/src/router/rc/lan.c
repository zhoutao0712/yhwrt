...
/* [MUST]: Need to discuss to add new mode for Media Bridge  */
	if(nvram_match("lan_proto", "static") || (nvram_get_int("sw_mode") == SW_MODE_AP && nvram_get_int("wlc_psta") == 0))
#else
...
NEITHER_WDS_OR_PSTA:
	...
	// Android phone, RNDIS interface, NCM, qmi_wwan.
		...
		if(!strcmp(action, "add")){
			...
			snprintf(buf, sizeof(buf), "%s", nvram_safe_get("usb_modem_act_path"));
			...
			snprintf(prefix, sizeof(prefix), "wan%d_", get_usbif_dualwan_unit());
...
#if defined(RTCONFIG_CHILLISPOT) || defined(RTCONFIG_COOVACHILLI)
					&& is_add_if(ifname)
#elif defined(RTCONFIG_RALINK) && defined(RTCONFIG_CONCURRENTREPEATER)
					&& (strncmp(ifname, "apcli", 5) && nvram_get_int("wlc_express") == 0)
#endif
				) {
					eval("brctl", "addif", lan_ifname, ifname);
#ifdef RTCONFIG_EMF
...
#ifdef RTCONFIG_REALTEK
/* [MUST]: Need to discuss to add new mode for Media Bridge  */
	if (nvram_get_int("sw_mode") == SW_MODE_AP && nvram_get_int("wlc_psta") != 1) { // AP mode
