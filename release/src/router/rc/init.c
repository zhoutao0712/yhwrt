...
misc_defaults(int restore_defaults)
{
	...
	switch(get_model()) {
		...
		case MODEL_RPAC53:
			...
			TRACE_PT("unknow MODEL for Realtek platform,[%d]\n", __LINE__);
			break;
#endif
		default:
			break;
...
/*#ifdef RTCONFIG_WIRELESSREPEATER
	...
	case MODEL_RPAC53:
		...
		nvram_set("lan_ifname", "br0");
		if (nvram_get_int("sw_mode") == SW_MODE_ROUTER)
			nvram_set("lan_ifnames", "wl0 wl1 wl0-vxd wl1-vxd wl0-wds0 wl1-wds0");
		else
			nvram_set("lan_ifnames", "wl0 wl1 eth1 wl0-vxd wl1-vxd wl0-wds0 wl1-wds0");
		nvram_set("wl0_ifname", "wl0");
		nvram_set("wl1_ifname", "wl1");
		if (nvram_get_int("sw_mode") == SW_MODE_ROUTER)
			nvram_set("wan_ifnames", "eth1");
...
#if !defined(RTAC1200GP) && !defined(RTAC51UP) 
...
int init_nvram2(void)
{
	...
	else if (nvram_get_int("sw_mode") == SW_MODE_AP && nvram_get_int("wlc_psta") == 1) {
		nvram_set_int("wlc_band", -1);
		nvram_set("wlc_ssid", "");
	}
#endif
...
#ifdef RTCONFIG_RREALTEK
/* [MUST]: Need to Clarify ... */
#define FT_CONFIG	"ft.conf"
			if(!strcmp(de->d_name, FT_CONFIG)){
				int status ;
				status = system("cp -r rom/etc/ft.conf tmp/");
				printf("ft.config status = %d\n", status);
			}
			else 
#endif /* RTCONFIG_RREALTEK */
			{
...
/*#ifdef RTCONFIG_USB_MODEM
			...
			if (nvram_match("Ate_power_on_off_enable", "0")) {	/* avoid run in test to let all led off */
				if (nvram_get_int("sw_mode") == SW_MODE_REPEATER ||
				    (nvram_get_int("sw_mode") == SW_MODE_AP && nvram_get_int("wlc_psta") == 1)) // Repeater and Media bridge mode
					set_led(LED_OFF_ALL, LED_OFF_ALL);
				else if (nvram_get_int("sw_mode") == SW_MODE_AP) { // AP mode
